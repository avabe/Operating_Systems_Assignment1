...
int poli = 1; //represents the current scheduling policy

long long tick = 1; //reprensents the time quantums
...
getMinAccumulator(void) {
    boolean pq_not_empty, rp_not_empty;
    long long runnables;
    long long runnings;
...
updatePerformance(struct proc *p, uint ticks) {
    ...
    //cprintf("thistick %d , old time %d, newTime %d \n ",ticks,p->oldTick,time);
...
exit(int status) {
   ...
   // rpholder.remove(curproc); not sure
...
getLongestIdleProc(void) {
    ...
    //find the first runnable in the process table
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->state == RUNNABLE) 
        {
            max=p;
            break;// we found the first one
        }
        
    }
    // for debugging because if we here than there is atleast one runnable
    if(!max)return 0;
    
    //find if there is another runnable that is idle than the last one
